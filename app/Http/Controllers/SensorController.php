...
class SensorController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        if (!Cache::has('averages')) {
            ...
            $averages['sensors']['today'] = SensorsValue::selectRaw('sensor_id, AVG(pm2_5) as pm2_5, AVG(pm10) as pm10, AVG(temperature) as temperature, AVG(humidity) as humidity, AVG(pressure) as pressure')->where('pm2_5', '>', 0)->where('pm10', '>', 0)->where('pressure', '>', 0)->where('created_at', '>=', Carbon::today())->groupBy('sensor_id')->get()->keyBy('sensor_id');
            $averages['sensors']['week'] = SensorsValue::selectRaw('sensor_id, AVG(pm2_5) as pm2_5, AVG(pm10) as pm10, AVG(temperature) as temperature, AVG(humidity) as humidity, AVG(pressure) as pressure')->where('pm2_5', '>', 0)->where('pm10', '>', 0)->where('pressure', '>', 0)->where('created_at', '>=', Carbon::today()->subDays(6))->groupBy('sensor_id')->get()->keyBy('sensor_id');
            $averages['sensors']['month'] = SensorsValue::selectRaw('sensor_id, AVG(pm2_5) as pm2_5, AVG(pm10) as pm10, AVG(temperature) as temperature, AVG(humidity) as humidity, AVG(pressure) as pressure')->where('pm2_5', '>', 0)->where('pm10', '>', 0)->where('pressure', '>', 0)->where('created_at', '>=', Carbon::today()->subMonth())->groupBy('sensor_id')->get()->keyBy('sensor_id');
            //select  FROM sensors_values GROUP BY month;
            $averages['chart']['previous'] = SensorsValue::selectRaw("AVG(pm2_5) as pm2_5,AVG(pm10) as pm10,DATE_FORMAT(created_at, '%m/%Y') as yearmonth,MONTH(created_at) as keyed,DATE_FORMAT(created_at, '%Y%m') as ordering")->where('created_at', '>', $previous_from)->where('created_at', '<=', $previous_to)->where('pm2_5', '>', 0)->where('pm10', '>', 0)->where('pressure', '>', 0)->orderBy('ordering', 'asc')->groupBy('ordering')->get()->keyBy('keyed');
            $averages['chart']['current'] = SensorsValue::selectRaw("AVG(pm2_5) as pm2_5,AVG(pm10) as pm10,DATE_FORMAT(created_at, '%m/%Y') as yearmonth,MONTH(created_at) as keyed,DATE_FORMAT(created_at, '%Y%m') as ordering")->where('created_at', '>', $current_from)->where('pm2_5', '>', 0)->where('pm10', '>', 0)->where('pressure', '>', 0)->orderBy('ordering', 'asc')->groupBy('ordering')->get()->keyBy('keyed');
